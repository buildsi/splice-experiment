...
that we planned to run for the BUILDSI project. This original (manual) file
is how we originally were testing (before we moved to a more automated approach).
...
# add anaconda to the path (where spliced is installed)
...
Install [spliced](https://github.com/buildsi/spliced), first install cle:
